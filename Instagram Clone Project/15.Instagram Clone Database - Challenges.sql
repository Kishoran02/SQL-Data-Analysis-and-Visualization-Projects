...
select tableA.table_A  as 'Number Of Users who never commented',
            (tableA.table_A / (select count(*) from users))*100 as '%',
            tableB.table_B as 'Number Of Users who commented on photos',
            (tableB.table_B / (select count(*) from users))*100 as '%'
from   (
			select count(*) as table_A 
            from (select users.username from users 
            left join comments on users.id = comments.user_id
            where comments.id is null) as total_number_of_users_without_comments
		) as tableA
        join 
        (    
			select count(distinct username) as table_B
            from (select users.username,comments.comment_text from users 
            left join comments on users.id = comments.user_id
            where comments.id is not null) as total_number_of_users_comments
		) as tableB;
